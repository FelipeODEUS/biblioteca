...
export default class TransactionScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if(!transactionType){
      ...
      Alert.alert(" O livro não existe no meu banco de dados")
  ...
  checkStudentEligibilityForBookIssue = async studentId => {
    ...
    if(studentRef.docs.length == 0){
      ...
      Alert.alert("O id do aluno não existe no meu banco de dados")
    } else{
      studentRef.docs.map(doc => {
        if(doc.data().number_of_books_issued < 2){
          ...
          isStudentEligible = false;
          Alert.alert(" o aluno ja pegou os 2 livros");
          this.setState({bookId: "", studentId: ""});
        }
      });
    }
  ...
  initiateBookIssue = async (bookId, studentId, bookName, studentName) => {
    ...
    //como alterar o status do livro
    db.collection("books")
    .doc(bookId)
    .update({
    ...
    //atualizar o estado local do app
    this.setState({
      bookId: "",
      studentId: ""
    })
